...
def analyze_invoice():
    ...
    #     "prebuilt-invoice", AnalyzeDocumentRequest(url_source=invoiceUrl), locale="en-US")

    file_path = Path(r"E:\document\CASH相关\Remittance文件\数据量大导致输出结果数据不完整\pdf\Mercadolibre (1).PDF")
    file_bytes = file_path.read_bytes()
    file_base64 = base64.b64encode(file_bytes).decode("utf-8")

    poller = client.begin_analyze_document(
        "prebuilt-invoice", AnalyzeDocumentRequest(bytes_source=file_base64), locale="en-US")
    ...
    if invoices.documents:
        for idx, invoice in enumerate(invoices.documents):
            ...
            if invoice.fields:
                ...
                if customer_id:
                    print(f"Invoice Id: {customer_id.get('content')} has confidence: {customer_id.get('confidence')}")
